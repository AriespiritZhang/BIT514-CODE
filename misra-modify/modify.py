...
KEYWORDS = {
                    ...
                    'union', 'unsigned', 'void', 'volatile', 'while','FILE',
                    'cout','cin','endl'
}
...
# 匹配misra.py文件自身编写了多少种规则 def 定义的在checker类当中
def getAddonRules():
    """Returns dict of MISRA rules handled by this addon."""
    addon_rules = []
    compiled = re.compile(r'.*def[ ]+gjb_([0-9]+)_([0-9]+)[(].*')
...
class MisraChecker:
    ...
    def gjb_1_1(self, data):
        for directive in data.directives:
            res = re.search(r'#define ([a-z][a-z0-9_]+)', directive.str)
            if res and (res.group(1) in KEYWORDS):
                self.reportError(directive, 1, 1)
    
    def gjb_1_2(self,data):
        for directive in data.directives:
            pattern = re.compile(r'#\s*define\s+([A-Za-z_]\w*)\s+(.*)')
            match = pattern.match(directive.str)
            if not match:
               continue
            value = match.group(2)
            for keyword in KEYWORDS:
                if keyword in value.split():
                    self.reportError(directive,1,2)

    def gjb_1_3(self,rawTokens):
        count = 0
        mytype = {}
        flag = False
        ...
        for v in values:
            if v > 1:
                self.reportError(tok,1,3)
    
    def gjb_1_4(self,data):
        for tok in data.tokenlist:
            if tok.isName and tok.str in KEYWORDS and tok.variable:
                self.reportError(tok,1,4)
                
    def gjb_1_5(self,data):
        define_pattern = re.compile(r'#define\s+([a-zA-Z]+\w+)\s+\w+')
        undef_pattern = re.compile(r'#undef\s+([a-zA-Z]\w+)')
        define_dic = {}
        for directive in data.directives:
            define_match = define_pattern.match(directive.str)
            undef_match = undef_pattern.match(directive.str)
            if define_match:
                try:
                    define_dic[define_match.group(1)] += 1
                    if define_dic[define_match.group(1)] > 1:
                        self.reportError(directive, 1, 5)
                except:
                    define_dic[define_match.group(1)] = 1
            if undef_match:
                try:
                    define_dic[undef_match.group(1)] -= 1
                except:
                    self.reportError(directive, 1, 6)
                    
    def gjb_1_6(self, data):
        # define_pattern = re.compile(r'#define\s+([a-zA-Z]+\w+)\s+\w+')
        # undef_pattern = re.compile(r'#undef\s+([a-zA-Z]\w+)')
            # match = define_pattern.match(tok.str)
            # if match:
                # print(tok.str)
        return
    
    # def gjb_1_7(self,data):
    #     pattern = re.compile(r'#define\s+[a-zA-Z]+\w+\(\S*\)\s+(.*)')
    #     parma_pattern = re.compile(r'[a-zA-Z]+\w*')
    #     for directive in data.directives:
    #         match = pattern.match(directive.str)
    #         if match:
    #             # print(match.start(1))
    #             pos = match.start(1)
    #             match_2 = parma_pattern.finditer(match.group(1))
    #             if match_2:
    #                 for match in match_2:
    #                     print(match.)
    #                 # if match_2[0] not in KEYWORDS:
    #                 #     if directive.str[pos-1]!= "(":
    #                 #         self.reportError(directive, 1, 7)
    #                 # else:
                        
    #     return
    
    def gjb_1_8(self,rawTokens):
        sp_list = ["struct", "union", "enum"]
        name_pattern = re.compile(r'[a-zA-z]\w*')
        for tok in rawTokens:
            if tok.str in sp_list and tok.previous.str != "typedef":
                match = name_pattern.match(tok.next.str)
                if not match:
                    self.reportError(tok, 1, 8)                

    def gjb_1_9(self,data):
        left = 0
        name_pattern = re.compile(r'[a-zA-z]\w*')
        for tok in data.tokenlist:
            if tok.str == "struct":
                if left > 0:
                    match = name_pattern.match(tok.next.next.link.next.str)
                    if match == None:
                        self.reportError(tok, 1, 9)
                if tok.next.next.str == "{":
                        left += 1
            if tok.str == "}":
                left -= 1
                if left < 0:
    ...
    def gjb_1_10(self, rawTokens):
        left = 0
        for i, item in enumerate(rawTokens):
            if item.str == "struct":
                if item.next.next.str ==  "{":
                        left += 1
                if left > 0:
                    for tok in rawTokens[i:]:
                        if tok.str == "signed":
                            if int(tok.next.next.next.next.str) < 2:
                                self.reportError(tok, 1, 10)
            if item.str == "}":
                left -= 1
                if left < 0:
                    left = 0
    
    def gjb_1_11(self, rawTokens):
        ...
        for i, item in enumerate(rawTokens):
            if item.str == "struct":
                ...
                if left > 0:
                    for tok in rawTokens[i:]:
                        if tok.str in integer and tok.next.next.str == ":":
                            if tok.previous.str not in flag:
                                self.reportError(tok , 1, 11)
            if item.str == "}":
                left -= 1
                if left < 0:
                    left = 0
    ...
    def gjb_1_13(self, data, rawTokens):
        left = 0
        name_pattern = re.compile(r'[a-zA-Z]\w*')
        for tok in data.tokenlist:
            if tok.function and tok.next.str == "(":
                left+=1
                continue
            if left > 0 and tok.str in KEYWORDS:
                match = name_pattern.match(tok.next.str)
                if not match:
                    self.reportError(tok, 1, 13)
            elif tok.str == "(" and tok.next.str == ")":
                line = tok.linenr
                column = tok.column
                for i in rawTokens:
                    if i.linenr == line and i.column == column:
                        if i.previous.str == "main":
                            break
                        else:
                            self.reportError(i, 1, 13)
    ...
    def gjb_1_15(self, data):
        name_pattern = re.compile(r'[a-zA-Z]\w*')
        for tok in data.tokenlist:
            if tok.function and tok.next.str == "(":
                if tok.next.next.str not in KEYWORDS:
                    match = name_pattern.match(tok.next.next.str)
                    if match:
                        self.reportError(tok, 1, 15)
    
    # def gjb_1_16(sekf, data):
    
    def gjb_1_17(self, data):
      outter = data.tokenlist[0].scope.Id
      extern_pattern = re.compile(r'extern\s+(\S+)\s+[a-zA-Z]\w*')
      for tok in data.tokenlist:
          summary = ""
          if tok.str == "extern":
            summary = "extern " + tok.next.str + " " + tok.next.next.str
            match = extern_pattern.match(summary)
            if match:
                if tok.scope.Id != outter:
                    self.reportError(tok, 1, 17)
    
    def gjb_1_18(self, rawTokens):
        array_pattern = re.compile(r'[a-zA-Z]\w+\s+[a-zA-Z]\w*\s*\[\]')
        for tok in rawTokens:
            summary = ""
            if tok.str in KEYWORDS:
                summary = tok.str + " " + tok.next.str + " " + tok.next.next.str + tok.next.next.next.str
                match = array_pattern.match(summary)
                if match:
                    self.reportError(tok, 1, 18)
                    
    def gjb_1_19(self, rawTokens):
        extern_pattern = re.compile(r'extern\s*([a-zA-Z]\w+)\s+[a-zA-Z]\w*\s*=\s*')
        for tok in rawTokens:
            summary = ""
            if tok.str == "extern":
                summary = tok.str + " " + tok.next.str + " " +  tok.next.next.str + " " + tok.next.next.next.str
                match = extern_pattern.match(summary)
                if match:
                    self.reportError(tok, 1, 19)
    
    def gjb_1_20(self, rawToekns):
        sign = ["signed", "unsigned"]
        for tok in rawToekns:
            if tok.str == "char" and tok.previous.str not in sign:
                self.reportError(tok, 1, 20)
            
    def gjb_1_21(self, data):
        win_absolute_path = re.compile(r'#include\s*\S+[a-zA-Z]\w*:\\')
        linux_absolute_path = re.compile(r'#include\s*\S+\/')
        for directive in data.directives:
            w_match = win_absolute_path.match(directive.str)
            lin_match = linux_absolute_path.match(directive.str)
            if w_match or lin_match:
                self.reportError(directive, 1, 21)
    
    def gjb_1_22(self, data):
        include_pattern = re.compile(r'#include\s*\S+(\S+)')
        include_map = {}
        for directive in data.directives:
            match = include_pattern.match(directive.str)
            if match:
                try:
                    include_map[match.group(1)] += 1
                    if include_map[match.group(1)] > 1:
                        self.reportError(directive, 1, 22)
                except:
                    include_map[match.group(1)] = 1
       
    def gjb_2_1(self, data):
        for tok in data.tokenlist:
            if tok.str == "for":
                outter = tok.scope.Id
                tok = tok.next.link
                if tok.next.scope.Id != outter and tok.next.str == "{":
                    if tok.next.column == 0:
                        self.reportError(tok, 2, 1)
                    else:
                        continue
    
    def gjb_2_2(self, data):
        eilist = ["if", "else if", "else"]
        for tok in data.tokenlist:
            if tok.str in eilist:
                outter = tok.scope.Id
                tok = tok.next.link
                if tok.next.scope.Id != outter and tok.next.str == "{":
                    if tok.next.column == 0:
                        self.reportError(tok, 2, 2)
                    else:
                        continue
                    
    def gjb_2_3(self, data):
        max = 0
        for directive in data.directives:
            if max < directive.linenr:
                max = directive.linenr
        if max > data.tokenlist[0].linenr:
            self.reportError(data.tokenlist[0], 2, 3)
            
    # def gjb_2_4(self, data): complex
        
    # def gjb_2_5(self, data): complex
    #    for tok in data.tokenlist:
    #        print(tok)
    
    def gjb_2_6(self, rawTokens):
        ps_pattern = re.compile(r'\/\*.+\/*\*\/')
        for tok in rawTokens:
            match = ps_pattern.match(tok.str)
            if match:
                self.reportError(tok, 2, 6)
    
    def gjb_3_1(self, data):
        for tok in data.tokenlist:
            if tok.valueType and tok.valueType.pointer > 2:
                self.reportError(tok, 3, 1)
                
    # def gjb_3_2  complex
    
    # gjb_3_3 cppcheck能够检测
    
    # gjb_3_5 cppcheck自检测
    
    #gjb_3_10 cppcheck 自检测 为资源泄露
    def gjb_4_1(self, data):
        for tok in data.tokenlist:
            if tok.str == "else" and tok.next.next.str == "if":
                tok = tok.next.link
                if tok.next.str != "else":
                    self.reportError(tok, 4, 1)
                    
    def gjb_4_2(self, data):
        for tok in data.tokenlist:
            if tok.str == "case" and tok.next.next.str == ":" and tok.next.next.next.str == ";":
                self.reportError(tok, 4, 2)
    
    def gjb_6_1(self, data):
        for tok in data.tokenlist:
            if tok.str == "=" and tok.isOp:
                if tok.previous.valueType:
                    if tok.previous.valueType.type=="int" and (tok.next.valueType.type == "double" or tok.next.valueType.type == "float"):
                        self.reportError(tok, 6, 1)
    ...
    # 6-6 cppcheck 自带
      # for tok in rawTokens:
            ...
            #     if not match:
            #         self.reportError(tok, 1, 13)
            # # elif tok.previous.str != "main" and tok.str == "(" and tok.next.str == ")":
            # #     self.reportError(tok, 1, 13)
            # if tok.next and tok.next.str == ")":
            #     left -= 1
            #     if left < 0:
    ...
    def reportError(self, location, num1, num2):
        ...
        if self.settings.verify:
            ...
            errorId = '8114-2013-' + str(num1) + '.' + str(num2)
            ...
            if ruleNum in self.ruleTexts:
                ...
                errmsg = 'GJB violation (use --rule-texts=<file> to get proper output)'
            else:
                return
            cppcheckdata.reportError(location, cppcheck_severity, errmsg, 'GJB', errorId, misra_severity)

            if misra_severity not in self.violations:
                self.violations[misra_severity] = []
            self.violations[misra_severity].append('GJB-' + errorId)
    # filename 是 rule-text文件
    def loadRuleTexts(self, filename):
        ...
        severity_pattern = re.compile(r'.*[ ]*(Advisory|Required)$')
    ...
    def parseDump(self, dumpfile):
        ...
        for cfg in data.configurations:
            ...
            self.executeCheck(101, self.gjb_1_1, cfg)
            self.executeCheck(102, self.gjb_1_2, cfg)
            if cfgNumber == 1:
                self.executeCheck(103, self.gjb_1_3, data.rawTokens)
            self.executeCheck(104, self.gjb_1_4, cfg)
            self.executeCheck(105, self.gjb_1_5, cfg)
            self.executeCheck(106, self.gjb_1_6, cfg)
            if cfgNumber == 1:
                self.executeCheck(108, self.gjb_1_8, data.rawTokens)
            self.executeCheck(109, self.gjb_1_9, cfg)
            if cfgNumber == 1:
                self.executeCheck(110, self.gjb_1_10, data.rawTokens)
                self.executeCheck(111, self.gjb_1_11, data.rawTokens)
                self.executeCheck(113, self.gjb_1_13, cfg, data.rawTokens)
            self.executeCheck(115, self.gjb_1_15, cfg)
            self.executeCheck(117, self.gjb_1_17, cfg)
            if cfgNumber == 1:
                self.executeCheck(118, self.gjb_1_18, data.rawTokens)
                self.executeCheck(119, self.gjb_1_19, data.rawTokens)
                self.executeCheck(120, self.gjb_1_20, data.rawTokens)
            self.executeCheck(121, self.gjb_1_21, cfg)
            self.executeCheck(122, self.gjb_1_22, cfg)
            self.executeCheck(201, self.gjb_2_1, cfg)
            self.executeCheck(202, self.gjb_2_2, cfg)
            self.executeCheck(203, self.gjb_2_3, cfg)
            if cfgNumber == 1:
                self.executeCheck(206, self.gjb_2_6, data.rawTokens)
            self.executeCheck(301, self.gjb_3_1, cfg)
            self.executeCheck(401, self.gjb_4_1, cfg)
            self.executeCheck(402, self.gjb_4_2, cfg)
            self.executeCheck(601, self.gjb_6_1, cfg)
            # self.executeCheck(107, self.gjb_1_7, cfg)
        ...
        if number_of_violations > 0:
            ...
            if settings.show_summary:
                ...
                for misra_id in sorted(rules_violated.keys(), key=misra_sort):
                    res = re.match(r'GJB8114-2013-([0-9]+)\\.([0-9]+)', misra_id)
                    if res is None:
                        num = 0
                    else:
